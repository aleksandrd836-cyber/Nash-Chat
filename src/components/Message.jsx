  ...
  return (
    <div className="group relative flex items-start gap-3 px-4 py-1 mt-2 hover:bg-ds-hover/30 rounded transition-colors animate-fade-in">
      ...
      <div className="w-[60px] h-[60px] rounded-full flex-shrink-0 bg-ds-bg shadow-[inset_0_0_10px_rgba(0,0,0,0.2)] overflow-hidden flex items-center justify-center">
        <img src={imageUrl} alt={realName} className="w-[90px] h-[90px] max-w-none select-none" />
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline gap-2">
          <span className="font-semibold text-sm" style={{ color: displayColor }}>{realName}</span>
          <span className="text-[10px] text-ds-muted" title={fullTime}>{time}</span>
        </div>
        {msg.content && (
          <div className="flex items-end gap-2 text-sm leading-relaxed">
            <p className="text-ds-text break-words whitespace-pre-wrap">{msg.content}</p>
            {isMine && isRead !== undefined && (
              <span className={`text-[11px] font-bold leading-none mb-1 select-none flex-shrink-0 ${isRead ? 'text-ds-accent' : 'text-ds-muted'}`}>
                {isRead ? '✓✓' : '✓'}
              </span>
