...
export function UserPanel({ username, userColor, onSignOut, voice, onOpenSettings }) {
  ...
  return (
    <div className="h-auto py-3 bg-black/40 flex-shrink-0 flex flex-col px-3 gap-3 border-t border-white/5 overflow-hidden relative">
      ...
      {/* Bottom Row: Controls */}
      <div className="flex items-center gap-1.5 bg-black/40 p-1 rounded-xl border border-white/5 shadow-2xl">
          ...
          {activeChannelId && (
            <>
              ...
              <button
                ...
                className={`flex-1 h-9 rounded-lg flex items-center justify-center transition-all duration-200 ${isMuted ? 'text-ds-red bg-ds-red/10 hover:bg-ds-red/20 vibe-glow-red animate-pulse' : 'text-white/40 hover:text-ds-accent hover:bg-ds-accent/10 hover:vibe-glow-blue'}`}
              >
                {isMuted ? (
              ...
              <button
                ...
                className={`flex-1 h-9 rounded-lg flex items-center justify-center transition-all duration-200 ${isDeafened ? 'text-ds-red bg-ds-red/10 hover:bg-ds-red/20 vibe-glow-red animate-pulse' : 'text-white/40 hover:text-ds-accent hover:bg-ds-accent/10 hover:vibe-glow-blue'}`}
              >
                {isDeafened ? (
                  <svg className="w-4.5 h-4.5" fill="currentColor" viewBox="0 0 24 24">
              ...
              <button
                ...
                className="flex-1 h-9 rounded-lg flex items-center justify-center text-white/20 hover:text-ds-red hover:bg-ds-red/10 transition-all duration-200"
              >
        ...
        {/* Separator if needed */}
        {activeChannelId && <div className="w-px h-4 bg-white/5 mx-0.5" />}

        {/* Settings */}
        <button
          onClick={onOpenSettings}
          title="Настройки"
          className="w-9 h-9 rounded-lg flex items-center justify-center text-white/20 hover:text-white hover:bg-white/10 transition-all duration-200 flex-shrink-0"
        >
