...
## 2026-04-10 orchestration layer extracted
- useVoice.js ?????? ?? ?????? ?????? ????????? inline cleanup ? sync peers loop.
- Cleanup ??????? ? src/hooks/voice/cleanup.js.
- Local presence/peer sync ??????? ? src/hooks/voice/sessionSync.js.
- ???????? ?????????? voice-?????? ? wiring signaling/status ???????? ? src/hooks/voice/localChannelBootstrap.js.
- ?? ???? ????? ???????? structural refactor voice-hook ????? ??????? ???????????; ?????? ????????????? ?????? ???????? ???????? ? ????? ????????? ?????.
